...
2차 커밋(수정 회차, 같은 날 같은 세션 — 사용자 지시): 사용자가 고른 결함 1·2·3 + 개선안 ①②④ + 날짜 열 `createdAt` 전환 + 속도 S2·S5(i) + 개정안 10 + frontmatter `name` 따옴표를 저장소 `SKILL.md`(495→563행, `wc -l`)에 반영, 점검표 개정안 1~14 를 `audit/checklist.md`(v3, 381→441행)에 반영, 이 파일에 "수정 기록" 절 추가. **재패키징·재업로드는 하지 않았다** — 검증은 다른 세션에서 받고, 그 뒤 사용자가 직접 한다. 그 전까지 설치본은 진단 시점 버전(md5 `abeb17eb…`)이다.
